...
class Chatbot_Utils:
    def __init__(self, user_id: str):
        ...
        self.ensure_directories_exist('main.ipynb', ['./temporary_image', './gpt_log'])

    def ensure_directories_exist(self, base_script: str, sub_dirs: List[str]) -> None:
        """
        確認temporary_image和gpt_log是否存在資料夾，若不存在就各別建立
        ...
        for directory in directories:
            if not os.path.exists(directory):
                ...
                print(f'資料夾 {directory} 不存在，創建資料夾')
            else:
                pass
    ...
    def fetch_tire_data(self, make: str, model: str, logger: logging.Logger) -> Any:
        """
        撈取輪胎資料(車輛資訊)
    ...
    def fetch_tire_specification_data(self, specification: str) -> Any:
        """
        撈取輪胎資料(只有輪胎規格)
    ...
    def fetch_tire_brand_data(self, brand: str, brand_model: str) -> Any:
        """
        撈取輪胎資料(輪胎品牌+輪胎型號)
        ...
        ragic_df['數量'] = pd.Categorical(ragic_df['數量'], categories=stock_order, ordered=True)
        ragic_df = ragic_df.sort_values(by=['類別', '數量']).dropna(subset=['數量'])
        filtered_df = ragic_df[
            (ragic_df['品牌英文'].isin([brand.upper()]) & ragic_df['品牌型號'].isin([brand_model])) |
            (ragic_df['品牌中文'].isin([brand]) & ragic_df['品牌型號'].isin([brand_model]))
        ][tire_data_columns].to_dict('records')
        return [dict(item) for item in filtered_df]

    def fetch_tire_brand_specification_data(self, brand: str, specification: str) -> Any:
        """
        撈取輪胎資料(輪胎品牌+輪胎規格)
        ...
        ragic_df = ragic_df.sort_values(by=['類別', '數量']).dropna(subset=['數量'])
        filtered_df = ragic_df[
            (ragic_df['品牌英文'].isin([brand]) & ragic_df['輪胎規格'].isin([specification])) |
            (ragic_df['品牌中文'].isin([brand]) & ragic_df['輪胎規格'].isin([specification]))
        ][tire_data_columns].to_dict('records')
        return [dict(item) for item in filtered_df]

    def fetch_tire_all_data(self, brand: str, brand_model: str, specification: str) -> Any:
        """
        ...
        ragic_df = ragic_df.sort_values(by=['類別', '數量']).dropna(subset=['數量'])
        filtered_df = ragic_df[
            (ragic_df['品牌英文'].isin([brand.upper()]) & 
            ragic_df['品牌型號'].isin([brand_model.upper()]) & 
            ragic_df['輪胎規格'].isin([specification])) | 
            (ragic_df['品牌中文'].isin([brand]) & 
            ragic_df['品牌型號'].isin([brand_model.upper()]) & 
            ragic_df['輪胎規格'].isin([specification]))
        ][tire_data_columns].to_dict('records')
        return [dict(item) for item in filtered_df]

    def fetch_tire_single_brand_data(self, brand: str) -> Any:
        """
        撈取輪胎資料(只有輪胎品牌)
    ...
    def fetch_tire_single_brand_model_data(self, single_brand_model: str) -> Any:
        """
        撈取輪胎資料(只有輪胎型號)
    ...
    def fetch_bolt_pattern_data(self, data: Dict[str, str], logger: logging.Logger) -> Any:
        """
        撈取鋁圈資料(車輛資訊)
    ...
    def fetch_bolt_pattern_specification_data(self, specification: str) -> Any:
        """
        撈取鋁圈資料(鋁圈規格)
    ...
    def parse_json_column(self, column: pd.Series) -> Any:
        """
        撈取委修單欄位資料
    ...
    def extract_fields_with_default(self, parsed_column: List[Any], fields: List[str]) -> Any:
        """
